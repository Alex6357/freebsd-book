...
`rm` 即 “Remove”，删除文件或目录。例如 `rm file`，`file` 是你想删除的文件或目录。如果目录里面有其他文件，则可以使用 `rm -r dir` 递归删除目录和其中的所有东西。使用 `rm -f file` 强行且不提示的删除某些被保护的文件。`rm -r` 和 `rm -f` 可以合写为 `rm -rf`。由于 `rm` 不带有回收站机制，使用的时候要尤其小心。
