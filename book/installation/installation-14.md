...
![设置主机名称](./assests/14-set-hostname.png "设置主机名称")
...
![正在校验组件](./assests/14-verify-distribution.png "正在校验组件")

![正在解压组件](./assests/14-extract-distribution.png "正在解压组件")
...
![UFS 有多块硬盘](./assests/14-ufs-multi-disks.png "UFS 有多块硬盘")
...
![正在分区](./assests/14-making-partitions.png)
...
![选择网络设备](./assests/14-choose-network-interface.png "选择网络设备")
...
![设置 IPv4](./assests/14-setup-ipv4.png "设置 IPv4")

选择是，会询问是否配置 DHCP。一般也选是。如果没有 DHCP 或想用静态 IP，则选否。

![配置 DHCP](./assests/14-setup-dhcp.png "配置 DHCP")

选择是，则会开始请求 DHCP。

![请求 DHCP](./assests/14-acquire-dhcp.png "请求 DHCP")
...
![设置 IPv6](./assests/14-set-ipv6.png "设置 IPv6")

如果选择是，同样会询问是否自动获取 IPv6，也可以手动配置。

![是否自动获取 IPv6 地址](./assests/14-set-slaac.png "是否自动获取 IPv6 地址")

![自动获取 IPv6 地址](./assests/14-acquire-slaac.png "自动获取 IPv6 地址")
...
![配置 DNS](./assests/14-set-dns.png "配置 DNS")

### 时区和时间设置

网络配置后会进入时区和时间设置。根据所在地时区进行选择。例如中国的大部分地区需要选择 `Asia` `China` `Beijing Time`。

![时区选择](./assests/14-time-zone-1.png "时区选择")

![时区选择](./assests/14-time-zone-2.png "时区选择")

![时区选择](./assests/14-time-zone-3.png "时区选择")

`CST` 即 `China Standard Time`，中国标准时间。回车确认。

![时区选择](./assests/14-time-zone-4.png "时区选择")

接下来可以设置当前时间。没有问题的话回车跳过两次。

![日期设置](./assests/14-set-date.png "日期设置")

![时间设置](./assests/14-set-time.png "时间设置")

### 服务设置

下一步是服务设置。

![服务设置](./assests/14-service-configuration.png "服务设置")
...
![添加用户](./assests/14-add-user.png "添加用户")
...
|  | 接下来是部分信息的确认。 |
| `OK?` | 确认是否创建用户。直接回车。 |
| `Add another user?` | 是否添加另一个用户。直接回车来退出。 |
...
![询问是否重启](./assests/14-reboot.png "询问是否重启")
